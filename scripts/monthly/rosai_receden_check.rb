...
class Receden_check < Receden_common
  ...
  def select_labor_sio(syocd,sryymd)
    ...
    if ret.empty?
      tbl=PandaTable.new(@db,"tbl_labor_sio")
      tbl["tbl_labor_sio.SYOCD"]     = syocd
      tbl["tbl_labor_sio.YUKOSTYMD"] = sryymd
      tbl["tbl_labor_sio.YUKOEDYMD"] = sryymd
      if tbl.execFunction("DBSELECT","key") == 0
        if tbl.execFunction("DBFETCH","key") == 0
          tbl.values.each{|key,value|
            ret[key.gsub(/^tbl_labor_sio\./,"")]=value
          }
  ...
  def main
    ...
    @hospnum,@recefile,@infile,@outfile,@errlog,@skykbn,@check_level=ARGV[0].split(",")
