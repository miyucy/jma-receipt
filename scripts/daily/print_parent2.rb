...
# 追加開始 (2002/09/17)
	if ls_w1 =~ /.red$/
		# .redファイルである
		red_file = base_dir + RED_DIR + ls_w1		# Redファイル名のセット(2002/10/16修正)
		lp_name = d2[30, 20].strip	# LP名のセット
		word3 = d2[51, (d2_len - 51)]	# 一時ファイルへ出力する内容のセット(2002/10/16修正)
...
# 追加終了(対応するendまで、インデントを追加している) (2002/09/17)
		dia_file = base_dir + DIA_DIR + ls_w1 + '.dia'		# Diaファイル名のセット(2002/10/16修正)
		def_file = base_dir + DEF_DIR + ls_w1 + '.def'		# Defファイル名のセット(2002/10/16修正)
		lp_name = d2[30, 20].strip	# LP名のセット
		word3 = d2[51, (d2_len - 51)]	# 一時ファイルへ出力する内容のセット(2002/10/16修正)
